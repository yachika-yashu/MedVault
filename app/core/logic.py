...
def clean_text(text: str) -> Tuple[str, CleaningMetadata]:
    ...
    # 4. De-hyphenation
    clean_text, n_dehyph = re.subn(r'(\w+)-\n(\w+)', r'\1\2', clean_text)
    stats["hyp"] = n_dehyph
...
async def extract_paper_metadata(openai_client, text_sample: str) -> PaperMetadata:
    """Uses LLM to extract semantic markers from any clinical document."""
    prompt = (
    ...
    try:
        ...
        return PaperMetadata(title="Unknown Document", authors=[], year=None)
