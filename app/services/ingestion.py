...
async def stream_process_ingestion(
    ...
    yield {"type": "progress", "value": 25, "message": "Analyzing document metadata..."}
...
async def ingest_text_direct(
    ...
    """
    Bypass PDF extraction — directly chunk, embed and index raw text.
    Used for PubMed full-text ingestion where we already have clean text.
    """
    from app.core.logic import clean_text, chunk_document, embed_chunks, estimate_cost
    ...
    chunks, _ = chunk_document(cleaned_text, doc_id, tenant_id, {})
    ingested_at = datetime.utcnow().isoformat()

    for c in chunks:
        c.metadata["filename"] = filename
        c.metadata["ingested_at"] = ingested_at
        c.metadata.update(metadata_override)
    ...
    db.commit()
    return {"filename": filename, "chunks": len(embedded)}
...
async def download_and_ingest_arxiv(
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            return f"Successfully ingested '{title}' (arxiv: {arxiv_id})"
